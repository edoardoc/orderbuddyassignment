...
export class JobService {
  ...
  async translateMenuJob(
  ): Promise<{ succeeded: boolean; message: string }> {
    
    this.logger.log(
      `Starting menu translation job.`,
    );
          ...
          if(location.supportedLanguages.length > 0){
            ...
            if(translatedFields.itemNames.length > 0 || translatedFields.itemDescriptions.length > 0) {
              menu.items.forEach((item: any, index: number) => {
                ...
                if (translatedFields.itemDescriptions[index] && translatedFields.itemDescriptions[index].translations.length > 0) {
            ...
            if(translatedFields.categoryNames.length > 0 || translatedFields.categoryDescriptions.length > 0) {
              menu.categories.forEach((category: any, index: number) => {
                if (translatedFields.categoryNames[index] && translatedFields.categoryNames[index].translations.length > 0) {
                ...
                if (translatedFields.categoryDescriptions[index] && translatedFields.categoryDescriptions[index].translations.length > 0) {
            ...
            if (Object.keys(updateSet).length > 1) { // needsTranslation is always there
              await this.menusCollection.updateOne(
                { _id: menu._id },
                { $set: updateSet }
              );
              ...
              this.logger.log(`Menu ID: ${menu._id} processed, no new translations applied, marked as not needing translation.`);
            }
              
          }else{
            this.logger.warn(`No supported languages found for menu ID: ${menu._id}, Location ID: ${locationId}, Restaurant ID: ${restaurantId}`);
          }

        } else {
          this.logger.warn(`Location not found or supportedLanguages missing for menu ID: ${menu._id}, Location ID: ${locationId}, Restaurant ID: ${restaurantId}`);
        }
      } else {
