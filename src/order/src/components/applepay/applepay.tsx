...
const ApplePay: React.FC<WalletsProps> = ({ amount, customerData, publicId, onPaymentComplete, onError, onCancel }) => {
    ...
    const initializeWallets = async () => {
      ...
      try {
        ...
        walletsRef.current.onerror = (error: any) => {
          console.log(error);
          logExceptionError(error, 'ApplePayProcessing', {
            operation: 'applePayWalletError'
          });
          onError?.(error);
        };
        ...
        logExceptionError(err, 'ApplePayInitialization', {
          operation: 'initializeApplePayWallet',
